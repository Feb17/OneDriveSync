...
class SyncManager: ObservableObject {
    ...
    func refreshRemotes() async {
        ...
        do {
            availableRemotes = try await rclone.listOneDriveRemotes()
        } catch {
    ...
    func addNewOneDriveRemote(name: String) async {
        do {
            try await rclone.configureNewOneDrive(name: name)
            // Refresh after delay
    ...
    func openFolderInOneDrive(_ folder: SyncFolder) async {
        ...
        if let rootURL = await rclone.getWebLink(remote: folder.remoteName, path: "") {
            await MainActor.run {
                _ = NSWorkspace.shared.open(rootURL)
            }
            return
        }
        
        if let fallbackURL = URL(string: "https://onedrive.live.com") {
            await MainActor.run {
